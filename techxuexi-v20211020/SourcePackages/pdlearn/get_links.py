...
def get_article_links():
    try:
        res = requests.get("https://www.xuexi.cn/lgdata/35il6fpn0ohq.json?_st=27718628&js_v=1657005134179")
        article = res.json()
...
def get_video_links():
    try:
        ...
        random.shuffle(link)
        '''
        res = requests.get("https://www.xuexi.cn/lgdata/1742g60067k.json?_st=27718675&js_v=1657005134179")
        article = res.json()
        titles = [t1['publishTime'] + t1['title'] for t1 in article]
        links = [t1['url'] for t1 in article]
        links = links[:200]
